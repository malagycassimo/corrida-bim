...
interface RestrictedCategoryModalProps {
    isOpen: boolean;
    onClose: () => void;
    categoryName?: string;
}

export function RestrictedCategoryModal({
    isOpen,
    onClose,
    categoryName,
}: RestrictedCategoryModalProps) {
    return (
        <AnimatePresence>
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
                    ...
                    >
                        ...
                        <h3 className="text-xl sm:text-2xl font-bold text-zinc-900 leading-snug">
                            Inscrição via Associação de Atletismo
                        </h3>

                        <p className="text-sm text-zinc-600 leading-relaxed px-2">
                            Não é possível continuar a inscrição no site para a categoria{" "}
                        ...
                        <div className="pt-2">
                            ...
                            >
                                Entendi / Escolher Outra Categoria
                            </button>
                        </div>
                    </motion.div>
                </div>
